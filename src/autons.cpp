...
void right_antenna_auto() {
  ...
  //move to three centered blocks
  chassis.pid_odom_set({{93_in, 48_in}, rev, 50}, true); //was93
  ...
  chassis.pid_odom_set({{112_in, 25_in}, rev, 127}, true); //was25,24
  chassis.pid_wait_quick_chain();

  //turn to face match loader
  MatchLoadDrop(true);
  chassis.pid_turn_set({112_in, 9_in}, rev, 127, true);
  chassis.pid_wait();

  //move into match loader and collect 3 blocks
  chassis.pid_odom_set({{111_in, 7.0_in}, rev, 127}, true); //was27,7
  chassis.pid_wait();
  pros::delay(250);

  //move forward into long goal and empty all blocks
  chassis.pid_odom_set({{111_in, 37_in}, fwd, 127}, true);
  chassis.pid_wait_until({111_in, 28_in});
  intakeTop.move(127);
  intakeBottom.move(127);
  chassis.pid_wait();
  // chassis.drive_angle_set(0_deg);
  ...
  //pull out of long goal
  chassis.pid_odom_set({{111_in, 29_in}, rev, 127}, true);
  chassis.pid_wait_quick_chain();
  ...
  //move away from wall
  chassis.pid_odom_set({{109.5_in, 29_in}, fwd, 127}, true);
  chassis.pid_wait_quick_chain();

  //turn so antenna is in goal
  chassis.pid_turn_set({109_in, 56_in}, fwd, 127, true);
  chassis.pid_wait();
  // chassis.pid_turn_set({108_in, 62_in}, rev, 127, true);
  // chassis.pid_wait_quick_chain();

  //push antenna to end of open area
  chassis.pid_odom_set({{109_in, 52_in}, fwd, 110}, true);
  // chassis.pid_wait_quick_chain();
