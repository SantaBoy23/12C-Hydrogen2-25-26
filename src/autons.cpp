...
void elims_left_auto() {
  ...
  chassis.pid_odom_set({{49_in, 52_in, 145_deg}, rev, 75}, true); //was90 speed //was80 speed //was150_deg
  ...
  chassis.pid_odom_set({{61.5_in, 59.5_in}, fwd, 127}, true); //was63,61,46
  ...
  intakeTop.move(-127);
  intakeBottom.move(127);
  chassis.pid_odom_set({{24.5_in, 26_in}, rev, 127}, true); //was24
  chassis.pid_wait();
  ...
  chassis.pid_odom_set({{24.5_in, 14.25_in}, rev, 127}, true); //was27,7
  chassis.pid_wait();
  pros::delay(300);

  //move forward into long goal and empty blocks
  chassis.pid_odom_set({{25_in, 41.5_in}, fwd, 127}, true);
  chassis.pid_wait_until({25_in, 32_in});
  intakeTop.move(127);
  intakeBottom.move(127);
  chassis.pid_wait();
  ...
  chassis.pid_odom_set({{26.5_in, 36_in}, rev, 127}, true);
  chassis.pid_wait_quick_chain();
  ...
  //move towards wall
  chassis.pid_odom_set({{19_in, 36_in}, fwd, 127}, true);
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_odom_set({{13.75_in, 58_in}, fwd, 127}, true);
  chassis.pid_wait();
...
void right_antenna_auto() {
  ...
  //move to three centered blocks
  chassis.pid_odom_set({{93_in, 48_in}, rev, 50}, true);
  ...
  chassis.pid_odom_set({{111.5_in, 28_in}, rev, 127}, true); //was25,24
  chassis.pid_wait_quick_chain();

  //turn to face match loader
  chassis.pid_turn_set({111.5_in, 9_in}, rev, 127, true);
  chassis.pid_wait();
  MatchLoadDrop(true);

  //move into match loader and collect 3 blocks
  chassis.pid_odom_set({{118_in, 12.35_in}, rev, 127}, true); //was27,7
  chassis.pid_wait();
  pros::delay(500);

  //move forward into long goal and empty all blocks
  chassis.pid_odom_set({{118_in, 43_in}, fwd, 127}, true);
  chassis.pid_wait_until({118_in, 32_in});
  intakeTop.move(127);
  intakeBottom.move(127);
  chassis.pid_wait();
  // chassis.drive_angle_set(0_deg);
  ...
  //pull out of long goal
  chassis.pid_odom_set({{119_in, 32_in}, rev, 127}, true);
  chassis.pid_wait_quick_chain();
  
  //turn away from wall
  // chassis.pid_turn_set(268_deg, TURN_SPEED); //was-1 //was 0
  // chassis.pid_wait();
  chassis.pid_turn_set({110.5_in, 32_in}, rev, 127, true);
  chassis.pid_wait_quick_chain();

  //move away from wall
  chassis.pid_odom_set({{117.5_in, 32_in}, fwd, 127}, true);
  chassis.pid_wait_quick_chain();
  ...
  //push antenna to end of open area
  chassis.pid_odom_set({{108_in, 60.5_in}, fwd, 127}, true);
  // chassis.pid_wait_quick_chain();
...
void sig_solo_awp (){ 
  ...
  //move into match loader and collect 3 blocks
  chassis.pid_odom_set({{125.25_in, 11.25_in}, rev, 100}, true); //was11.2
  chassis.pid_wait();
  pros::delay(100);

  //move straight back into long goal and deposit blocks
  chassis.pid_odom_set({{125.0_in, 39.25_in}, fwd, 127}, true);
  ...
  MatchLoadDrop(false);
  chassis.pid_odom_set({{125.0_in, 33_in}, rev, 127}, true);
  chassis.pid_wait();
  ...
  CenterDrop(false);
  intakeTop.move(0);
  chassis.pid_odom_set({{27.5_in, 19_in}, rev, 127}, true); //was25,24
  chassis.pid_wait();
  intakeTop.move(-127);
  ...
  // //move into match loader and collect 3 blocks
  chassis.pid_odom_set({{28.85_in, 4.15_in}, rev, 70}, true); //was5.2  //was5
  chassis.pid_wait();
  pros::delay(300);

  //move forward into long goal and empty all blocks
  chassis.pid_odom_set({{27.5_in, 39_in}, fwd, 127}, true);
...
void skills_auto() {
  ...
  //move towards match loader  
  chassis.pid_odom_set({{125.5_in, 22_in}, rev, 127}, true); //was90 speed //was80 speed //was150_deg
  chassis.pid_wait();
  ...
  chassis.pid_odom_set({{125.5_in, 11.85_in}, rev, 90}, true); //was13.5
  chassis.pid_wait();
  pros::delay(1400);

  //pull out, lift loader, and turn towards wall
  chassis.pid_odom_set({{125.5_in, 24_in}, fwd, 127}, true);
  chassis.pid_wait();
  MatchLoadDrop(false);
  ...
  intakeTop.move(0);
  intakeBottom.move(0);
  chassis.pid_odom_set({{135_in, 24_in}, fwd, 127}, true);
  chassis.pid_wait();
  ...
  //move away from wall
  chassis.pid_odom_set({{125_in, 110_in}, fwd, 127}, true);
  chassis.pid_wait();
  ...
  chassis.pid_odom_set({{123_in, 99.25_in}, fwd, 127}, true);
  ...
  MatchLoadDrop(true);
  intakeTop.move(-127);
  chassis.pid_odom_set({{123.25_in, 131.2_in}, rev, 127}, true);
  ...
  chassis.pid_odom_set({{122.0_in, 99.5_in}, fwd, 127}, true);
  ...
  chassis.pid_wait();
  chassis.drive_angle_set(180_deg);
  chassis.odom_xyt_set(122.0_in, 99.5_in, 180_deg);
  pros::delay(2100);
  MatchLoadDrop(false);

  //pull back from long goal and turn to face left side
  intakeTop.move(-127);
  chassis.pid_odom_set({{122_in, 110_in}, rev, 127}, true);
  chassis.pid_wait();
  // chassis.pid_turn_set(90_deg, TURN_SPEED);
  // chassis.pid_wait();
  chassis.pid_turn_set({26_in, 110_in}, rev, 127, true);
  chassis.pid_wait();

  //move to left side
  chassis.pid_odom_set({{28.0_in, 110_in}, rev, 127}, true); //was25.5
  chassis.pid_wait();
  ...
  //move into match loader and collect all blocks
  MatchLoadDrop(true);
  chassis.pid_odom_set({{28.750_in, 132.25_in}, rev, 70}, true);
  chassis.pid_wait();
  pros::delay(1400);

  //pull out of match loader and lift match load mech
  chassis.pid_odom_set({{28.750_in, 122_in}, fwd, 127}, true);
  chassis.pid_wait();
  MatchLoadDrop(false);
  ...
  intakeTop.move(0);
  intakeBottom.move(0);
  chassis.pid_odom_set({{18_in, 122_in}, fwd, 127}, true);
  chassis.pid_wait();
  ...
  //move to red side
  chassis.pid_odom_set({{17.25_in, 32_in}, fwd, 127}, true); //was 18
  chassis.pid_wait();
  ...
  //move towards center
  chassis.pid_odom_set({{28.25_in, 32_in}, fwd, 127}, true);
  chassis.pid_wait();
  ...
  chassis.pid_odom_set({{29_in, 46_in}, fwd, 127}, true);
  chassis.pid_wait();
  intakeBottom.move(127);
  intakeTop.move(127);
  pros::delay(2100);
  ...
  MatchLoadDrop(true);
  intakeTop.move(-127);
  chassis.pid_odom_set({{28.75_in, 11_in}, rev, 80}, true);
  chassis.pid_wait();
  pros::delay(1100);

  //move straight into long goal and deposit all blocks
  chassis.pid_odom_set({{28.25_in, 45.5_in}, fwd, 127}, true);
  chassis.pid_wait();
  intakeTop.move(127);
  pros::delay(2100);
  MatchLoadDrop(false);
  ...
  // //set starting angle
  // chassis.drive_angle_set(0_deg);
  // chassis.odom_xyt_set(30_in, 46_in, 0_deg);
  ...
  chassis.pid_odom_set({{56_in, 14_in, 270_deg}, rev, 90}, true);
  chassis.pid_wait();

  pros::delay(100);

  // chassis.pid_odom_injected_pp_set({{{30_in, 32_in}, rev, 110},  //was48.75
  //                                   {{42_in, 25_in}, rev, 110}, 
  //                                   {{56_in, 14_in, 270_deg}, rev, 110}}, 
  //                                   // {{57_in, 45_in}, fwd, 80}}, 
  //                                   true);
  // chassis.pid_wait();
